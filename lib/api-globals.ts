...
/**
 * Hydrate in-memory store from Supabase.
 * Call at the start of any route that needs agencies (match-agencies, distribute).
 * No-op when: store already populated, Supabase not configured, or fetch fails.
 */
export async function ensureAgenciesHydrated(): Promise<void> {
  const g = global as any
  // Already populated or hydration already attempted this process lifetime
  if (g[PB_AGENCIES_STORE]?.length > 0 || g['__PB_AGENCIES_HYDRATED__']) return
  ...
  try {
    const res = await fetch(
      `${url}/rest/v1/agencies?status=eq.active&is_active=eq.true&select=*`,
    ...
    for (const row of rows) {
      if (store.find(a => a.id === row.id)) continue // already exists
      store.push({
        id: row.id,
        ...
        buyer_markets: row.buyer_markets ?? [],
        languages: row.languages ?? [],
        specializations: row.specializations ?? [],
        delivery_channels: row.delivery_channels ?? ['email'],
        quality_score: row.quality_score ?? 72,
        historical: {
          response_rate: row.response_rate ?? 0,
          conversion_rate: row.conversion_rate ?? 0,
          avg_response_hours: row.avg_response_hours ?? 24,
          owner_direct_success_rate: row.owner_direct_success_rate ?? 0,
          cross_border_deals_12m: row.cross_border_deals_12m ?? 0,
          luxury_deals_12m: row.luxury_deals_12m ?? 0,
        },
        status: row.status,
        ...
        contact_policy: row.contact_policy ?? 'open',
      })
    }

    g['__PB_AGENCIES_HYDRATED__'] = true
    console.log(`[api-globals] Hydrated ${rows.length} agencies from Supabase`)
  } catch (e) {
    console.warn('[api-globals] Supabase hydration failed (non-fatal):', e)
  }
}
